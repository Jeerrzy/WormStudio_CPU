...
class TrajectoryOptimizer(object):
    ...
    def init_trajectories(self, trk_seq_path):
        ...
        if self.adaptive:
            total_ids = int(trk_data[:, 1].max())
            total_frames = int(trk_data[:, 0].max())
            distancePerFrame = []
            # ratioPerID = []
            for _id in range(1, total_ids + 1):
                id_centroid_data = trk_data[trk_data[:, 1] == _id][:, 2:4]
                # ratioPerID.append(len(id_centroid_data) / total_frames)
